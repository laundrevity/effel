...
fn fragment(in: FragmentInput) -> @location(0) vec4<f32> {
    let phi = in.uv.x * 2.0 * 3.14159265359;
    let theta = in.uv.y * 3.14159265359;
    let sin_theta = sin(theta);
    let x = sin_theta * cos(phi);
    let y = sin_theta * sin(phi);
    let z = cos(theta);

    let position_on_sphere = vec3<f32>(x, y, z);

	let noise_value = perlinNoise3(position_on_sphere * 10.0 + vec3<f32>(0.0, 0.0, sin(globals.time)));	
	let fire_intensity = clamp(noise_value * 1.5 + 0.2, 0.0, 1.0);

	// Use fire_intensity to blend between red and yellow
    let fire_color = mix(vec3<f32>(1.0, 0.0, 0.0), vec3<f32>(1.0, 1.0, 0.0), fire_intensity);

	return vec4<f32>(fire_color, 1.0);
}
